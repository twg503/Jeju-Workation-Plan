...
이 폴더를 Codex에서 열어 작업할 때는 다음과 같이 요청하면 된다.

> `JEJU.html을 수정하고 https://github.com/twg503/Jeju-Workation-Plan 저장소의 master 브랜치에 올린 뒤 GitHub Pages 배포까지 확인해줘.`

Codex는 다음 순서로 처리한다.

1. `JEJU.html`과 관련 문서를 수정한다.
2. HTML 안의 JavaScript 문법과 장소·콘텐츠 데이터 개수를 검사한다.
3. 원격 `master`를 임시 폴더에 새로 복제한다.
4. 수정한 파일만 복제본에 반영해 원격의 기존 이력과 `index.html`, `.nojekyll`을 보존한다.
5. 변경 파일을 확인한 뒤 커밋하고 `master`에 Push한다.
6. GitHub의 최신 커밋 SHA와 Pages 응답 상태를 확인한다.
7. 배포 페이지에서 새 레이어와 장소명이 실제로 포함됐는지 검사한다.

현재 작업 폴더가 원격 저장소와 다른 Git 이력을 가지고 있거나 원격 주소가 설정되지 않은 경우에는 강제 Push하지 않는다. 원격 저장소를 새로 복제한 뒤 필요한 파일만 옮겨 커밋하는 방식으로 기존 GitHub 이력을 보호한다.

GitHub 인증이 만료되면 Git Credential Manager의 브라우저 로그인 창이 표시될 수 있다. 이때 `twg503` 계정으로 승인한다. 비밀번호나 개인 액세스 토큰을 채팅, HTML, Markdown 파일에 직접 적지 않는다.

배포 완료 기준은 다음과 같다.

- GitHub 저장소의 `master` 브랜치에 새 커밋이 존재한다.
- `https://twg503.github.io/Jeju-Workation-Plan/JEJU.html` 요청이 HTTP 200으로 응답한다.
- 배포된 HTML 용량과 주요 장소·레이어 문자열이 새 파일과 일치한다.
- 검증용 임시 복제본이 삭제된다.
